...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 28 16:22:30 2021
...
def get_neighbors(training_dataset, test_dataset, K_value):
    manhattan_distance_list = list()
    for row in training_dataset:
        euclid_distance_value = calculate_manhattan_distance(test_dataset, row)
        manhattan_distance_list.append((row, euclid_distance_value))
    manhattan_distance_list.sort(key=lambda tup: tup[1]) 
    neighbors_list = list()
    for i in range(1, K_value):
        neighbors_list.append(manhattan_distance_list[i][0])
    return neighbors_list

# calculates the euclidian distance
def calculate_manhattan_distance(test_dataset, row):
    manhattan_distance = 0.0
    for i in range(len(test_dataset)-1):
        manhattan_distance = manhattan_distance + abs((test_dataset[i] - row[i]))
...
for selected_k_value in range (10,26):
    ...
    plt.show()
    accuracy_values[selected_k_value-10] = accuracy_first
    accuracy_values_diag[selected_k_value-10] = accuracy_second
    f_score_values[selected_k_value-10] = FScore6
